...
// Phase 34.6 CHECKPOINT REWORK (LEN-01/LEN-02/KEY-02): level-02 REDESIGNED after the
// 34.6-04 human checkpoint REJECTED the flat first cut ("no climbing at all, no going
// backwards at all"). It is now a real, TALL, MANDATORY switchback climb bridging a
// 1160px pit (F4→F5), a deliberate descent down the far side, and an OPTIONAL math-skip
// KEY at the up-left apex (KS). NO geometry.locks (math-skip = keys without locks). See
// src/levels/level-02.js's header for the full design rationale; the rejected-flat-cut
// and pre-Phase-24 historical values are no longer reproduced here — see git history.
{
  ...
  const expectedGeometry = {
    ...
    platforms: [
      { x: 280, y: 254, w: 120, h: 16 },
      { x: 3220, y: 246, w: 300, h: 16 },
      { x: 3450, y: 172, w: 280, h: 16 },
      { x: 3660, y: 98, w: 300, h: 16 },
      { x: 3420, y: 24, w: 350, h: 16 },
      { x: 3700, y: -50, w: 340, h: 16 },
      { x: 3470, y: -124, w: 300, h: 16 },
      { x: 4160, y: 135, w: 340, h: 16 },
    ],
    coins: [
      { x: 150, y: 264 },
      { x: 450, y: 264 },
    ...
    spikes: [
      { x: 1660, y: FLOOR_Y - CONFIG.SPIKE_SIZE },
      { x: 2320, y: FLOOR_Y - CONFIG.SPIKE_SIZE },
      { x: 4910, y: FLOOR_Y - CONFIG.SPIKE_SIZE },
      { x: 6380, y: FLOOR_Y - CONFIG.SPIKE_SIZE },
      { x: 7060, y: FLOOR_Y - CONFIG.SPIKE_SIZE },
    ],
    ...
    mathGates: [],
    enemies: [
      { x: 5620, y: FLOOR_Y - CONFIG.ENEMY.H, variant: 0 },
    ],
    // Phase 34.6 (KEY-02/LEN-02) — the math-skip KEY, no locks. Sits on the optional
    // up-LEFT key apex tier KS (x:3470..3770, y:-124).
