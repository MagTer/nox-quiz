...
- [x] **VALID-03**: Interactive audit drives start→goal with mechanic encounters on all 8 levels (harness upgraded to shrink the current 6/16 encounter blind spot)
...
- [x] **VIS-02**: Expanded grunge palette — hue-tinted darks (moss green, blue-grey, rust), WCAG AA contrast recorded per role, no pink
- [x] **VIS-03**: Per-level background/accent theme tinting via the art pipeline, under human visual sign-off
- [x] **VIS-04**: Doors and enemies get real CC0 sprite art replacing their placeholder rect+glyph rendering (config.js's enemy `COLOR` is literally commented "placeholder"; door is a flat grey panel), under human visual sign-off — added 2026-07-07 per user request during Phase 26 discuss, echoing the original v5.0 kickoff framing ("make sure monster, doors and other parts of the game are reviewed and updated")
