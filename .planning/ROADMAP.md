...
- [x] 34.6-07-PLAN.md — Rebuild level-06 (cemetery, intense/vertical + math-skip key) — LEN-01, LEN-02
- [x] 34.6-08-PLAN.md — Rebuild level-07 (castle intro, monotonic staircase — deletes the 5 headroom fails) — LEN-01, LEN-02
- [x] 34.6-09-PLAN.md — Rebuild level-08 (castle finale, switchback + math-skip key) — LEN-01, LEN-02

**Wave 5** *(serialized after 34.6-05 — shared smoke-progress fixture)*

- [x] 34.6-10-PLAN.md — Rebuild level-04 (town, intense/vertical + math-skip key) + smoke re-baseline — LEN-01, LEN-02
...
- [x] 35-01-PLAN.md — Props infrastructure: CONFIG.PROPS z tunables + collider-free build.js emit loop + main.js load branch + geometry-freeze gate/baseline + screenshot script (no art yet)
- [x] 35-02-PLAN.md — 2-level trial: bake swamp+cemetery props, dress level-01 + level-06, capture trial screenshots
- [x] 35-03-PLAN.md — Human-verify checkpoint (BLOCKING, non-rubber-stamped) on the 2-level trial look; gates the remaining-6 rollout
- [x] 35-04-PLAN.md — Dress level-02 (swamp) + level-05 (cemetery) reusing the trial-baked, approved art
- [x] 35-05-PLAN.md — Bake town (pre-sliced) + castle (tileset crops first, CC0 where thin, licenses/credits) prop art
- [x] 35-06-PLAN.md — Dress town level-03 + level-04 (level-04 minimal — object budget)
- [x] 35-07-PLAN.md — Dress castle level-07 + level-08 with spawn+climb screenshots
- [x] 35-08-PLAN.md — Consolidation: delete dead theme-N bake code, restyle coin-collect pop(), full suite + frozen-geometry proof + all-8 spot-check screenshots
...
- [x] 36-01-PLAN.md — Geometry-freeze reconciliation: exclude motion keys from the freeze snapshot (RED-first)
- [x] 36-02-PLAN.md — Interactive audit: ride every mover + cross every patroller — dispatch in audit-retry.mjs, assertive RED-first fixture
- [x] 36-10-PLAN.md — Motion art assets: swamp + cemetery light-source props (missing biomes) + distinct walk-animated patroller sprite; lanterns placed in level-01/06 (BEFORE the trial)

**Wave 2**

- [x] 36-03-PLAN.md — Mover (dt-sine, native carry) + patroller (patrol, respawn-hazard) entity classes + config

**Wave 3**

- [x] 36-04-PLAN.md — Ambient flicker/unlock anims + MECH-05 alcove persistent torch (derived lit-on-entry)

**Wave 4**

- [x] 36-05-PLAN.md — Trial: motion on level-01 (calm) + level-06 (intense) + motion-evidence screenshots

**Wave 5**

- [x] 36-06-PLAN.md — Human sign-off: hazard placement on the 2 trial levels (SC5, non-rubber-stamped)

**Wave 6** *(parallel — file-disjoint)*

- [x] 36-07-PLAN.md — Rollout: motion on levels 02/03/04 (calm/intense rhythm)
- [x] 36-08-PLAN.md — Rollout: motion on levels 05/07/08 (all 8 levels now moving)

**Wave 7**

- [x] 36-09-PLAN.md — Consolidation: full CLAUDE.md gate suite green + audit rides/crosses all 8, geometry byte-frozen
...
- [x] 37-01-PLAN.md — RED-first Playwright touch-coordinate probe (proves the current CSS-transform desync, ~480 vs ~320) — MOB-01

**Wave 2** *(blocked on Wave 1)*

- [x] 37-02-PLAN.md — Letterbox migration (main.js + index.html #stage container): probe flips GREEN, desktop parity gate green, stale comments rewritten — MOB-01

**Wave 3** *(parallel — file-disjoint; all blocked on 37-02)*

- [x] 37-03-PLAN.md — Input seam (src/input.js) + player.js wiring, keyboard byte-identical, CONFIG.TOUCH tunables — MOB-02
- [x] 37-04-PLAN.md — Portrait "rotate your device" overlay + gesture suppression (touch-action:none, viewport meta; no orientation.lock) + machine-asserted portrait/landscape overlay probe (touch-orientation-probe) — MOB-04
- [x] 37-05-PLAN.md — Tappable UI proof (tap resolves answer + mute + arms/confirms the title reset, added tappable in title.js keyboard byte-identical) + audio gesture-gate wiring + iOS ITP doc — MOB-03, MOB-05

**Wave 4** *(blocked on 37-03)*

- [x] 37-06-PLAN.md — Touch controls UI (coarse-pointer-only left/right/jump, per-identifier multi-touch, variable-height via the seam) + touch-drive harness — MOB-02

**Wave 5** *(blocked on 37-04/05/06)*

- [x] 37-07-PLAN.md — Consolidation: full gate suite green (existing + 3 new touch gates) + 37-VERIFICATION.md (MOB-05 device audio + MOB-06 deferred to Phase 38) — MOB-01..05
...
- [x] 39-07-PLAN.md — Level 8 data: ground skeletons (POL-01), movers over the real moat + chasm with stepping-stones removed (POL-03 FROZEN), columns off gaps/spawn (POL-05)
