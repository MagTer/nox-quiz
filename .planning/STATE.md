...
- **Biome parallax "black mess" regression found + fixed, 2026-07-12, at the Phase 33 human-verify checkpoint** (commit `caebfae`): `_bake_biome_parallax_layer()` in `scripts/build-art-assets.py` was crushing the rich Gothicvania biome background art to near-solid `#0a0a0a` via a `_remap_luminance()` pass borrowed from the flat-Kenney-silhouette ground/tile pipeline — never present in the actually-approved `styleboard.py` reference. Fixed by dropping that remap (retint/tile/anchor/save only, matching `styleboard.py`); all 12 biome parallax PNGs regenerated from re-fetched CC0 sources (`CREDITS.md` documents them as re-fetchable), full 9-command gate suite reverified green including `check-pink-gate.sh`. User's explicit instruction (2026-07-11 night): investigate/fix the root cause and get back to the Phase 33 checkpoint autonomously without reopening Phase 31/32 planning or asking further questions — only the final human sign-off itself stays a real pause.
