...
| VIS-02 | Every named palette role clears WCAG AA (4.5:1 text / 3:1 UI); no hue in the banned magenta/pink band | automated + manual (human sign-off, per 26-CONTEXT.md's "in addition to human sign-off" mandate) | `node scripts/check-contrast.mjs` (NEW) for the WCAG/banned-hue math; `26-02` Task 5 human checkpoint against `26-PALETTE-SWATCH.png` for the subjective pink/magenta/mauve judgment | ❌ Wave 0 |
...
|----------|-------------|------------|--------------------|
| Palette banned-hue sign-off | VIS-02 | Subjective color-perception judgment ("does this read as pink/magenta/mauve to a human eye") — the automated `isBannedHue()` channel-delta math is a useful proxy but 26-CONTEXT.md explicitly requires human sign-off "in addition to" it, since desaturated magenta/mauve can still visually read as pink in ways a fixed numeric threshold may miss | View `26-PALETTE-SWATCH.png` (or the swatch rendered in-game), look at every labeled role including ACCENT_MOSS/ACCENT_SLATE/ACCENT_RUST, confirm none reads as pink/magenta/mauve |
