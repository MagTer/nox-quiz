...
- [x] 08-02-PLAN.md — Game-feel + presentation: variable jump height / coyote / buffer, frame-rate-independent clamped camera, gentle checkpoint respawn (MOVE-03, MOVE-04, LEVEL-06)
...
- [x] 09-02-PLAN.md — Load CC0 sprites, author level data + merged-floor builder, widen bounds, sprite-swap player; one traversable dark/grunge level with reliable collision (LEVEL-01, LEVEL-02, LEVEL-03)
...
- [x] 10-02-PLAN.md — Build src/ui/mathGate.js: the single in-world gate bridge (fixed/dimmed overlay, dual input, forgiving wrong, no-timer) + structural check script (GATE-01, GATE-04, GATE-05)

**Wave 3** *(blocked on Wave 2)*

- [x] 10-03-PLAN.md — Wire onReachGoal() in src/scenes/game.js to openMathGate (fresh per-scene brain, reuse player-freeze, onClear clears the level) + wiring check script (GATE-03)
...
- [x] 11-01-PLAN.md — Wave 1: pure src/progress.js (XP/level math + guarded localStorage seam)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 11-02-PLAN.md — Wave 2: brain seedAccuracy+snapshot, onClear({table}), scene load/award/persist wiring
