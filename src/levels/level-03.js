...
export const LEVEL_03 = {
  ...
  props: [
    ...
    { sprite: "prop-town-barrel", x: 960, y: 290, layer: "surface", solid: true, solidW: 24, solidH: 30 }, //  POL-04 SOLID jump-over — F1 (moved from 880: a solid prop must NOT overlap a checkpoint respawn point — the player would respawn INSIDE the collider and death-loop; checkpoint@880, so 960 keeps a 64px gap), before the door@1060; solidW/H = town-barrel.png's TRUE 24x30 (WYSIWYG collider == art, 2026-07-20), 30px << 88px jump envelope
    ...
    { sprite: "prop-town-barrel", x: 3400, y: 290, layer: "surface", solid: true, solidW: 24, solidH: 30 }, //  POL-04 SOLID jump-over — F4 left (past the AW2 landing ~3360), before coin@3480 + spike@3580; solidW/H = town-barrel.png's TRUE 24x30 (WYSIWYG)
    { sprite: "prop-town-crate", x: 5340, y: 285, layer: "surface", solid: true, solidW: 39, solidH: 35 }, //  POL-04 SOLID jump-over — F6 (moved from 5420: a solid prop must sit >150px BEFORE a platform-mount takeoff or it lands in the walk-driver's hop-suppression zone and deadlocks the goal-drive; DW1 takeoff@~5540, so 5340+39 keeps 161px clearance), clear of coin@5240 + checkpoint@5160; solidW/H = town-crate.png's TRUE 39x35 (WYSIWYG collider == art, 2026-07-20)
